...
from models.common2 import *
from models.experimental_effnet import *
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, C3, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3TR, C3SPP, C3Ghost]: #  C3,
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, C3Ghost]: #  C3,
                args.insert(2, n)  # number of repeats
                n = 1
        elif m is nn.BatchNorm2d or m is nn.GroupNorm:
            args = [ch[f]]
        elif m is Concat:
