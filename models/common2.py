...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        # self.att = TransformerLayer(c_, 4) 

        # self.att_layer = nn.Sequential( 
        #         nn.Linear(c_, c_, bias=False),
        #         nn.Linear(c_, c_, bias=False),
        #         nn.Linear(c_, c_, bias=False),
        #         nn.MultiheadAttention(embed_dim=c_, num_heads=4),
        #         nn.Linear(c_, c_, bias=False),
        #         nn.Linear(c_, 2*c_, bias=False)
        #                  )

    def forward(self, x):
        y = torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1)
        # print("y shape = ", y.shape)
        # # y0 = self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1))
        # # print("y shape = ", y.shape)
        # att = self.att(y[0,:,:,:])
        # print("Attent out = ", att.shape)
        # out = att.expand(1, att.shape[0], att.shape[1], att.shape[2])
        # print(out.shape)
        out = self.cv3(y)
        # print("out = ", out.shape)
        return out
...
class C3TR(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        c_ = int(c2 * e)
        self.m = TransformerBlock(c_, c_, 2, n) # orignal head = 4
